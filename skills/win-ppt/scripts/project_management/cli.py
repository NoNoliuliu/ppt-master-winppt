...
""",
    ...
    subparsers.add_parser("list-templates", help="List the four local Weining templates")
    local_template = subparsers.add_parser("import-template", help="Install a local Weining template")
    local_template.add_argument("project_path")
    local_template.add_argument("template_name")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command in {"list-templates", "import-template"}:
            from win_templates import list_templates, import_template
            if args.command == "list-templates":
                list_templates()
                return 0
            return import_template(args.project_path, args.template_name)
        ...
        if args.command == "import-sources":
            summary = manager.import_sources(
                ...
                args.sources,
                move=args.move,
                copy=args.copy or not args.move,
