...
For a two-page structured-format example, read [`examples/structured-spec-lock.md`](./examples/structured-spec-lock.md). It demonstrates the required `communication` section, exact Master/Layout names from the installed template SVG metadata, and the `images` row syntax. Before using it in a project, replace its content values with confirmed Design Spec decisions, place the selected prototype SVGs in `<project_path>/templates/`, place every declared image in `<project_path>/images/`, and register each placed image in Design Spec §VIII. A user-stated prohibition may be added under `forbidden` only as the exact user wording followed by `(user)`; do not invent one merely to fill the section.

Before Executor begins a structured deck, check the layout keys and PowerPoint names against the actual template SVG metadata; ensure `page_pptx_layouts` and `page_layouts` cover every `page_rhythm` page; ensure every placed logo/image appears in §VIII and in the lock's `images` section; and declare recurring typography size roles before their third use. During page checks, repair overflowing group bounds at the page and its declared bounds rather than weakening the gate. Use `Native-ready: no` for hand-drawn SVGs without a chart/table JSON source; editable SVG-to-PPTX shapes do not require `Native-ready: yes`.
