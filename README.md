...
本仓库中的 Obsidian 发布规则用于维护者的正式源管理，他人使用生成能力不需要接入维护者的个人仓库；更新时应获取新分享版本并自行保留本地配置。

普通默认生成会在绘制第一页前自动尝试启动本地实时预览，并尝试打开浏览器、报告预览链接。预览服务启动失败时，改用聊天确认并继续基本 PPTX 制作；明确要求预览的任务需先解决预览故障。Quick 流程不强制启动这项实时预览。
